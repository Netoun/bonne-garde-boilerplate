#!/usr/bin/env bun
/**
 * Initialise le projet pour le développement local :
 *   1. Sélection interactive des apps/packages à conserver (+ cleanup CI/docs)
 *   2. Copie des .env.example / .dev.vars.example → .env / .dev.vars
 *   3. Génération BETTER_AUTH_SECRET si placeholder
 *   4. Migrations D1 locales + seed
 *
...
function parseFlags(): Flags {
  ...
  return {
    ...
    skipDb: has("skip-db"),
    keep: keepArg ? keepArg.slice("--keep=".length).split(",").map((s) => s.trim()) : null,
    remove: removeArg ? removeArg.slice("--remove=".length).split(",").map((s) => s.trim()) : null,
  };
}
...
function promptSelection(available: { apps: string[]; packages: string[] }): {
  ...
  const selected = new Set<string>(
    allModules.map((m) => `${m.type}:${m.name}`),
  );
  ...
  while (true) {
    ...
    if (trimmed === "a") {
      const allSelected = numbered.every((n) =>
        selected.has(`${n.type}:${n.name}`),
      );
...
function confirmRemoval(toRemove: { apps: string[]; packages: string[] }): boolean {
  ...
  console.log("");
  console.log("  Keep:     " + (toRemove.apps.length > 0 || toRemove.packages.length > 0 ? "as selected" : "everything"));
  if (toRemove.apps.length > 0)
    console.log("  Remove apps:     " + toRemove.apps.join(", "));
...
function cleanupEnvironmentMd(removed: string[]): void {
  if (removed.length === 0) return;
  const path = join(ROOT, "docs/ENVIRONMENT.md");
  if (!existsSync(path)) return;
  ...
  if (changed) {
    if (flags.dryRun) {
      console.log("  [dry-run] Would update docs/ENVIRONMENT.md");
    } else {
      writeFileSync(path, content);
      done("Updated docs/ENVIRONMENT.md");
    }
  }
}

function cleanupArchitectureMd(removed: string[]): void {
  if (removed.length === 0) return;
  ...
  if (changed) {
    if (flags.dryRun) {
      console.log("  [dry-run] Would update docs/architecture.md");
    } else {
      writeFileSync(path, content);
      done("Updated docs/architecture.md");
    }
  }
}
...
function main() {
  ...
  if (flags.skipApps) {
    ...
    if (toRemoveApps.length > 0 || toRemovePackages.length > 0) {
      ...
      const removedAppNames = allRemoved
        .filter((r) => r.type === "app")
        .map((r) => r.name);
      ...
      cleanupCommandsMd();
      cleanupEnvironmentMd(removedAllNames);
      cleanupArchitectureMd(removedAppNames);
  ...
  if (!flags.dryRun) {
    saveState({
      ...
      phases: {
        ...
        envSetup: flags.skipEnv
          ? loadState()?.phases.envSetup ?? "pending"
          : "done",
        dbSetup: flags.skipDb
          ? loadState()?.phases.dbSetup ?? "pending"
          : "done",
      },
    });
  }
