...
| Rich editor | TipTap block Notion-like (`packages/editor`) |
...
├── packages/editor   # TipTap block Notion-like
...
└── task/             # Migration tasks (1 file = 1 task)
```
...
bun run lint && bun run fmt               # oxlint + oxfmt
...
- **Read before writing**: this file → the task → `git show main:<path>` (legacy) → existing code
...
- **Strict scope**: one task = one scope, do not touch elsewhere "in passing"
- **Mark Done**: `## ✅ Done` at the end of task file when finished
...
- ❌ Skip phase in `AGENTS.md#recommended-execution-order`
...
- [ ] New env vars → [`ENVIRONMENT.md`](./docs/ENVIRONMENT.md)
